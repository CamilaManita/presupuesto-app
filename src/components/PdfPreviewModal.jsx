...
export const PdfPreviewModal = ({
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      // 3. Enviar copia escrita por correo directamente vía API de Resend
      ...
      fetch('https://api.resend.com/emails', {
        ...
        .then(data => {
          if (data.id) {
            ...
            alert('¡PDF descargado y copia de presupuesto enviada a presupuestovidrieriavallcanera@gmail.com!');
          } else {
            console.warn('Respuesta de Resend:', data);
            alert('PDF descargado. Aviso de correo: ' + (data.message || data.name || JSON.stringify(data)));
          }
        })
        .catch(err => {
          console.error('Error al conectar con Resend:', err);
          alert('PDF descargado. Nota: No se pudo conectar con el servicio de correo.');
        });
